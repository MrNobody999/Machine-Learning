...
# Load the video
video_path = "car_moving4.mp4"  # Change to your video file path
...
parked_threshold = 50  # Frames for considering a vehicle parked
...
def is_stable(track):
    movements = [get_direction(track[i], track[i + 1]) for i in range(len(track) - 1)]
    return all(movement == "stable" for movement in movements)
    ...
    if frame_count % frame_interval == 0:
        ...
        for car_id, track in current_frame_tracks.items():
            if len(track) == min_frames_for_decision:
                if is_stable(track):
                    continue  # Skip stable vehicles

                direction = get_direction(track[0], track[-1])
                if direction in ["right to left", "upper right to lower left", "top to bottom"]:
                    if not is_vehicle_near(track[-1], resized_frame.shape) and len(track) < parked_threshold:
